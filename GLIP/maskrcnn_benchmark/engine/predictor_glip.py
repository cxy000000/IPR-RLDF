...
class GLIPDemo(object):
    ...
    def build_tokenizer(self):
        cfg = self.cfg
        tokenizer = None
        if cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE == "/data/chenxy/models/bert-base-uncased":
            print(1)
            tokenizer = AutoTokenizer.from_pretrained("/data/chenxy/models/bert-base-uncased")
        ...
        for entity, label in zip(relevant_phrases, labels):
            try:
                ...
                for m in re.finditer(entity, caption.lower()):
                    # print(entity)
                    # the start position and the end position of each entity in the image
    ...
    def run_ner_with_entity(self, caption, custom_entity):
        relevant_phrases = custom_entity
        labels = custom_entity
        self.entities = custom_entity
        tokens_positive = []
        for entity, label in zip(relevant_phrases, labels):
            try:
                for m in re.finditer(entity, caption.lower()):
    ...
    def run_on_web_image(self, 
        ...
        top_predictions, scores, labels = self._post_process(predictions, thresh)
        # print("scores is :", scores)
        # print("labels are :", labels)
        print("top_predictions are :", top_predictions)
        result = original_image.copy()
        if self.show_mask_heatmaps:
            return self.create_mask_montage(result, top_predictions)
        result = self.overlay_boxes(result, top_predictions)
        result, name_labels, glip_boxes, glip_scores = self.overlay_entity_names(result, top_predictions)
        if self.cfg.MODEL.MASK_ON:
            result = self.overlay_mask(result, top_predictions)
        return result, top_predictions, name_labels, glip_boxes, glip_scores
    ...
    def compute_prediction(self, original_image, original_caption, custom_entity = None):
        ...
        if isinstance(original_caption, list):
            ...
            if custom_entity is None:
                ...
                tokens_positive = self.run_ner_with_entity(original_caption, custom_entity)
    ...
    # added parameter custom_entity, when not None, could be used to designate the choice and neglect threshold
    def _post_process(self, predictions, threshold=0.5):
        ...
        labels = predictions.get_field("labels").tolist()
        _, idx = scores.sort(0, descending=True)
        return predictions[idx], scores, labels
    ...
    def overlay_entity_names(self, image, predictions, names=None, text_size=1.0, text_pixel=2, text_offset = 10, text_offset_original = 4):
        ...
        return image, new_labels, boxes, scores
